...
- **`json:` searches the raw embedded workflow.** The parsed fields are a summary — prompt, model, sampler, a few numbers — so node class types, user-typed node titles, upscale model names and custom-node widget values were unreachable. `json:` reaches all of it, composes with every other operator (`json:UltimateSDUpscale AND -json:karras`), and is first in the advanced panel's **Search in** list. Aliases: `raw` `meta` `metadata` `workflow` `node`.
- **Build JSON index**, in ⚙ advanced search. Newly scanned files are indexed for free during the normal parse; only files cached by an earlier build need this one-time pass, and a `json:` search that can't see everything says so rather than returning a misleading empty list.

### Fixed
- **"Show all" in the People view no longer forces list view.** A grid user was dropped into list on every trip through People. Every other exit from an excursion view already honoured the remembered view; this one path hardcoded list.
- The "Processing…" overlay's text was dark-on-dark in light mode. Its backdrop is a fixed near-black in both themes, so its text no longer takes the theme's colours.
...
- **☰ Hide the directories panel**, from the toolbar or the `B` key, persisted as `guru-sidebar`. The preference is tracked separately from the `.collapsed` class the detail view already uses, so opening and closing an image no longer resets it.
...
- **Export / import the AI index**, from the AI panel. Moves an analyzed library to a second machine instead of re-running hours of GPU work — one JSON file carrying every embedding, face and person name. Matching on the far end is by path, validated against the file's timestamp; the report says how many matched, how many paths weren't found and how many had moved on, so a library that doesn't line up says so instead of failing quietly. Two imports are refused outright: a different model version, and a different precision tier (fp16 vectors and int8 vectors are different embedding spaces, and every score is packed into one matrix with no provenance — mixing them corrupts the ranking without ever throwing).
